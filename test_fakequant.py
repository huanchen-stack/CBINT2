...
print("\n=== Benchmark: 128 layers of 3x(2048x768) tensors ===")
import time
...
for _ in range(num_tensors):
    ...
    scale = torch.ones((bench_out, bench_in // 16), dtype=torch.float32)
    gscale = torch.ones((1,), dtype=torch.float32)
    layers.append((packed, scale, gscale))
...
for p, s, g in layers_d:
    _ = q.fakequant_layer(p, s, g)
torch.cuda.synchronize()
elapsed = time.perf_counter() - t0

blocks_per_tensor = (bench_out * bench_in) // 16
total_blocks = blocks_per_tensor * num_tensors
print(f"  cuda: {elapsed:.2f}s  ({num_tensors} tensors, {total_blocks} total blocks)")
