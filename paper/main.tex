...
Notably, RaZeR observes that ``lookup-based formats such as Normal-Float and Student-Float store the lookup table in FP16, necessitating high-precision MAC operations with significant energy overhead,'' concluding that ``the FP4-E2M1 format offers a favorable trade-off between model accuracy and hardware efficiency''~\cite{chen2026razer}.
CBINT2 delivers exactly this trade-off at sub-4-bit rates: our codebook entries \emph{are} FP4-E2M1 values, so dequantization feeds tensor cores directly with no FP16 intermediate.
